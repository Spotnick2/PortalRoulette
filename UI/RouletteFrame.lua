...
function RouletteFrame:ResetVortexState(visible)
    ...
    resetTextureVisual(vortex.outerWisps, 0.14 * alpha, 1, 0)
    resetTextureVisual(vortex.innerSpiral, 0.24 * alpha, 1, 0)
    resetTextureVisual(vortex.glowBloom, 0.12 * alpha, 1, 0)
    resetTextureVisual(vortex.motes, 0.10 * alpha, 1, 0)
    vortex.outerAngle = 0
    vortex.innerAngle = 0
    vortex.moteAngle = 0
...
function RouletteFrame:PlayVortexOpen()
    ...
    ns.Animations:Play(vortex.outerWisps, 0, 0.14 * intensity, 0.36, 1.0, 0.62)
    ns.Animations:Play(vortex.glowBloom, 0, 0.18 * intensity, 0.18, 1.0, 0.30, function()
    ...
    if ns.db and ns.db.portalVortexMotesEnabled ~= false then
        ns.Animations:Play(vortex.motes, 0, 0.10 * intensity, 0.85, 1.0, 0.54)
    end
end
...
function RouletteFrame:PlayVortexCollapse()
    ...
    collapse(vortex.outerWisps, 0.18 * intensity, 0.42, 0.22)
...
function RouletteFrame:CreateWheel()
    ...
    self.vortex.backSmoke = createVortexTexture(ns.Media.PORTAL_VORTEX_BACK_SMOKE, WHEEL_SIZE + 86, "BACKGROUND", "BLEND", { 0.56, 0.66, 1.0, 1 })
    self.vortex.outerWisps = createVortexTexture(ns.Media.PORTAL_VORTEX_OUTER_WISPS, WHEEL_SIZE + 54, "BORDER", "ADD", { 0.40, 0.54, 1.0, 1 })
    self.vortex.innerSpiral = createVortexTexture(ns.Media.PORTAL_VORTEX_INNER_SPIRAL, WHEEL_SIZE + 18, "BORDER", "ADD", { 0.50, 0.66, 1.0, 1 })
    self.vortex.glowBloom = createVortexTexture(ns.Media.PORTAL_VORTEX_GLOW_BLOOM, WHEEL_SIZE - 64, "BORDER", "ADD", { 0.42, 0.70, 1.0, 1 })
    self.vortex.motes = createVortexTexture(ns.Media.PORTAL_VORTEX_MOTES, WHEEL_SIZE + 42, "BORDER", "ADD", { 0.60, 0.70, 1.0, 1 })
